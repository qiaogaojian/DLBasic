...
public class TestReporter : MonoBehaviour
{
	...
	void Start()
	{
		...
		XDebug.Log("test long text sdf asdfg asdfg sdfgsdfg sdfg sfg" +
		...
		for (int i = 0; i < 10; i++) {
			XDebug.Log("Test Collapsed log");
			XDebug.LogWarning("Test Collapsed Warning");
			XDebug.LogError("Test Collapsed Error");
		}
	...
	void threadLogTest()
	{
		for (int i = 0; i < threadLogTestCount; i++) {
			XDebug.Log("Test Log from Thread");
			XDebug.LogWarning("Test Warning from Thread");
			XDebug.LogError("Test Error from Thread");
		}
	}
	...
	void Update()
	{
		...
		while (currentLogTestCount < logTestCount && drawn < 10) {
			XDebug.Log("Test Log " + currentLogTestCount);
			XDebug.LogError("Test LogError " + currentLogTestCount);
			XDebug.LogWarning("Test LogWarning " + currentLogTestCount);
			drawn++;
			currentLogTestCount++;
		}
